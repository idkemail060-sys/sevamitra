  ...
  try {
    ...
    if (error) {
      ...
      if (error.message && (error.message.includes('relation "public.bookings" does not exist') || error.code === '42P01')) {
        ...
        syncStatus.lastError = 'Connected to database endpoint, but "bookings" table not created yet. Please execute the provided SQL script in Supabase SQL editor.';
        notifySyncChange();
    ...
    return {
      ok: true,
      message: `Successfully connected to Supabase! Live database verified (${bRes.count ?? 0} bookings, ${uRes.count ?? 0} users, ${wRes.count ?? 0} workers).`,
    };
  } catch (err: any) {
    syncStatus.isConnected = false;
