...
export const AdminSettings: React.FC = () => {
  ...
  const [config, setConfig] = useState<MatchingWeightsConfig>({
    categoryWeight: state.config?.weightCategoryMatch ?? 0.4,
    localityWeight: state.config?.weightLocalityMatch ?? 0.3,
    ratingWeight: state.config?.weightRating ?? 0.15,
    availabilityWeight: state.config?.weightAvailability ?? 0.15,
    fairnessWeight: state.config?.fairnessBonusMultiplier ?? 0.25,
    weeklyJobThreshold: state.config?.maxJobsPerWeekThreshold ?? 10,
    platformFeePercent: state.config?.platformFeePercent ?? 5,
    cooperativeFundPercent: state.config?.cooperativeFundPercent ?? 2,
  });
  const [savedSuccess, setSavedSuccess] = useState(false);

  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    store.updateMatchingWeights({
      platformFeePercent: config.platformFeePercent,
      cooperativeFundPercent: config.cooperativeFundPercent,
      weightCategoryMatch: config.categoryWeight,
      weightLocalityMatch: config.localityWeight,
      weightRating: config.ratingWeight,
      weightAvailability: config.availabilityWeight,
      fairnessBonusMultiplier: config.fairnessWeight,
      maxJobsPerWeekThreshold: config.weeklyJobThreshold,
    });
