...
export const AuthPage: React.FC<AuthPageProps> = ({
  ...
  // Step 3: Admin Specific Credentials
  ...
  const handleFastDemoLogin = (userId: string, targetRole: UserRole) => {
    if (targetRole === 'ADMIN') {
      ...
      setRole('ADMIN');
      setFullName('Rajeshwari Rao');
      setContactInput('rajeshwari.admin@sevamitra.coop');
      setAdminPassword('');
  ...
  const handleDetailsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === 'WORKER') {
        ...
        // ADMIN password validation: must be 123456
        if (adminPassword.trim() !== '123456') {
          setAdminPasswordError('Incorrect Cooperative Admin password. Please enter 123456.');
          ...
          <div className="space-y-6">
            ...
            <div className="pt-2 flex items-center justify-between text-xs text-slate-500 border-t border-slate-100">
              ...
              <button
                ...
                onClick={() => {
                  setRole('ADMIN');
                  if (!fullName) setFullName('Rajeshwari Rao');
                  if (!contactInput) setContactInput('rajeshwari.admin@sevamitra.coop');
                  setAdminPassword('');
                  setAdminPasswordError(null);
                  setCurrentStep('DETAILS');
                }}
                className="text-indigo-600 hover:text-indigo-800 font-semibold cursor-pointer underline flex items-center gap-1.5"
              >
                <Lock className="w-3.5 h-3.5 text-indigo-600" />
                <span>Log in as Cooperative Admin</span>
              </button>
            </div>
          ...
          <form onSubmit={handleDetailsSubmit} className="space-y-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                >
                  {role === 'HOUSEHOLD' ? 'Household Profile' : role === 'WORKER' ? 'Worker Profile' : 'Admin Credentials'}
                </span>
                <h3 className="text-lg font-bold text-slate-900">
                  {role === 'HOUSEHOLD'
                    ...
                    : 'Cooperative Administrator Verification'}
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  Contact: <strong className="text-slate-800">{contactInput || (role === 'ADMIN' ? 'rajeshwari.admin@sevamitra.coop' : '')}</strong>
                </p>
              </div>
              <button
                type="button"
            ...
            {/* =========================================================================
                ADMIN PASSWORD VERIFICATION (PASSWORD: 123456)
            ========================================================================= */}
            {role === 'ADMIN' && (
              <div className="space-y-4 pt-1">
                <div className="p-4 bg-indigo-50/90 border border-indigo-200 rounded-2xl text-xs text-indigo-950 space-y-2 shadow-xs">
                  <div className="font-bold flex items-center justify-between text-indigo-900">
                    <span className="flex items-center gap-2 text-sm">
                      <ShieldCheck className="w-4 h-4 text-indigo-600" />
                      <span>Cooperative Society Admin Verification</span>
                    </span>
                    <span className="text-[10px] font-bold bg-indigo-200 text-indigo-800 px-2 py-0.5 rounded-full uppercase tracking-wider">
                      Protected
                    </span>
                  </div>
                  <p className="text-slate-600 leading-relaxed text-xs">
                    You are accessing administrative credentials for the <strong>Karnataka State Gig Workers Cooperative Society</strong> (Reg #COOP-2026-KA).
                  </p>
                </div>

                <div>
                  <label className="text-xs font-semibold text-slate-700 block mb-1">
                    Administrator Official ID
                  </label>
                  <input
                    type="text"
                    readOnly
                    value={contactInput || 'rajeshwari.admin@sevamitra.coop'}
                    className="w-full px-3 py-2.5 text-xs rounded-xl border border-slate-200 bg-slate-100 text-slate-700 font-mono cursor-not-allowed"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={fullName || 'Rajeshwari Rao'}
                    onChange={(e) => setFullName(e.target.value)}
                    placeholder="Rajeshwari Rao"
                    className="w-full px-3 py-2.5 text-xs rounded-xl border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none text-slate-900 bg-white"
                  />
                </div>

                {/* Cooperative Admin Password input */}
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                      <Lock className="w-3.5 h-3.5 text-indigo-600" />
                      <span>Cooperative Admin Password</span>
                      <span className="text-red-500">*</span>
                    </label>
                    <span className="text-[11px] text-slate-500">
                      Master Key: <code className="bg-slate-100 text-indigo-700 font-bold px-1.5 py-0.5 rounded font-mono border border-slate-200">123456</code>
                    </span>
                  </div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Enter admin password (123456)"
                      className={`w-full pl-9 pr-10 py-2.5 text-sm rounded-xl border transition font-mono ${
                        adminPasswordError
                  ...
                  {adminPasswordError && (
                    <div className="mt-2 flex items-center gap-1.5 text-xs text-rose-600 font-semibold animate-in fade-in duration-200">
                ...
                <div className="p-3 bg-amber-50/80 border border-amber-200 rounded-xl text-[11px] text-amber-900 flex items-start gap-2">
                  ...
                  <span>
                    Confidential privilege grants authority to approve worker KYC verification, inspect algorithm fairness, review union welfare fund accounts, and settle grievances.
                  </span>
                </div>
              </div>
            )}
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-xl font-bold text-sm shadow-md transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 ${
                role === 'ADMIN'
            ...
            >
              <span>
                {isSubmitting
                  ...
                  : role === 'ADMIN'
                  ? 'Verify Password & Log in as Admin'
                  : `Submit Details & Enter as ${role === 'HOUSEHOLD' ? 'Household' : 'Worker'}`}
              </span>
              <ArrowRight className="w-4 h-4" />
            </button>
          </form>
