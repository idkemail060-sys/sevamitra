...
export const DemoSwitcher: React.FC<DemoSwitcherProps> = ({
  ...
  const handleSelectPersona = (persona: (typeof personas)[0]) => {
    if (persona.role === 'ADMIN' && state.currentUserId !== persona.id) {
      const enteredPwd = window.prompt('Enter Cooperative Admin Password (password is 123456):');
      if (enteredPwd !== '123456') {
        if (enteredPwd !== null) {
          alert('Incorrect Cooperative Admin password. Access denied.');
        }
        return;
      }
    }
    ...
    if (onRoleChanged) {
      onRoleChanged(persona.role as any);
    }
    safeNavigate(persona.destination);
    setIsOpen(false);
  };
  ...
  const handleResetData = () => {
    if (window.confirm('Reset all demo bookings, votes, and grievances to initial SIH seed state?')) {
      ...
      safeNavigate('/household/dashboard');
    }
  };
    ...
    <div className="sticky top-0 z-50 bg-slate-900 text-white border-b border-slate-800 shadow-md text-xs sm:text-sm">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-1.5 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Supabase Connection Status Button (Hidden) */}
          <button
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-medium px-2.5 sm:px-3 py-1 rounded-md shadow-sm transition text-xs cursor-pointer"
          >
