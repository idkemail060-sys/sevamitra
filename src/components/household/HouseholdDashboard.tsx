...
export const HouseholdDashboard: React.FC<HouseholdDashboardProps> = ({
  ...
  // Filter household's bookings safely
  const householdId = currentUser?.id || state?.currentUserId || '';
  const householdBookings = (state?.bookings || []).filter(
    (b) => b && (b.householdId === householdId || (currentUser?.phone && b.householdPhone === currentUser.phone))
  );

  const activeBookings = householdBookings.filter(
    (b) => b && (b.status === 'REQUESTED' || b.status === 'ACCEPTED' || b.status === 'IN_PROGRESS')
  );
  ...
  const totalSpent = householdBookings
    .filter((b) => b && b.paymentStatus === 'CAPTURED')
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs bg-slate-50 p-3 rounded-xl border border-slate-200">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-emerald-600 text-white font-bold flex items-center justify-center text-sm">
                      {(b.workerName || 'Artisan').charAt(0)}
                    </div>
                    <div>
                      <span className="font-bold text-slate-900 block">{b.workerName || 'Assigned Artisan'}</span>
                      <span className="text-slate-500 text-[11px]">Assigned Cooperative Professional • {b.workerPhone || 'In dispatch'}</span>
                    </div>
                  </div>
              ...
              >
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500">
                    Worker: <span className="font-medium text-slate-700">{b.workerName || 'Artisan'}</span> • Date: {b.scheduledDate || 'Recent'}
                  </p>
