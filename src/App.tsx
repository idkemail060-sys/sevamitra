...
export default function App() {
  ...
  // Router State - Defaults directly to /login page as requested
  ...
  useEffect(() => {
    const cleanup = initSupabaseRealtime((remoteBooking) => {
      setTick((t) => t + 1);
      setToastMessage(`Supabase Live Sync: Booking ${remoteBooking.bookingCode} updated`);
  ...
  const handleRoleChanged = (_newRole: UserRole) => {
    // Open home page first after login as requested
    setCurrentPath('/');
  };
  ...
  // Auto-redirect authenticated users away from auth forms or generic dashboard
  useEffect(() => {
    if (currentUser) {
      if (
        currentPath === '/login' ||
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans antialiased selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <div className="flex-1 flex flex-col">
        {isDashboardView && currentUser ? (
          ...
          <div className="flex-1 max-w-7xl w-full mx-auto flex flex-col lg:flex-row min-w-0">
            ...
            <main className="flex-1 p-4 sm:p-6 lg:p-8 min-w-0">
              ...
              {currentUser.role === 'HOUSEHOLD' && (
                <ErrorBoundary fallbackTitle="Household Dashboard Error">
                  {currentPath === '/household/services' ? (
                    ...
                    />
                  ) : currentPath === '/household/payments' ? (
                    <HouseholdPayments />
                  ) : (
                    /* Default fallback for /household, /household/dashboard, /household/profile, etc. */
                    <HouseholdDashboard
                      onNavigate={handleNavigate}
              ...
              {currentUser.role === 'WORKER' && (
                <ErrorBoundary fallbackTitle="Worker Dashboard Error">
                  {currentPath === '/worker/requests' || currentPath === '/worker/bookings' ? (
                    <WorkerRequests />
                  ) : currentPath === '/worker/earnings' ? (
                    <WorkerEarnings />
                  ) : currentPath === '/worker/governance' ? (
                    <WorkerGovernance />
                  ) : currentPath === '/worker/grievances' ? (
                    <WorkerGrievances />
                  ) : (
                    /* Default fallback for /worker, /worker/dashboard, /worker/availability, etc. */
                    <WorkerDashboard
                      onNavigate={handleNavigate}
                      onOpenProfilePictureModal={() => setIsProfilePictureModalOpen(true)}
                    />
              ...
              {currentUser.role === 'ADMIN' && (
                <ErrorBoundary fallbackTitle="Admin Dashboard Error">
                  {currentPath === '/admin/verification' ? (
                    ...
                    <AdminSettings />
                  ) : currentPath === '/admin/governance' ? (
                    <AdminGovernance />
                  ) : currentPath === '/admin/reports' ? (
                    <AdminReports />
                  ) : currentPath === '/admin/grievances' ? (
                    <AdminGrievances />
                  ) : currentPath === '/admin/bookings' || currentPath === '/admin/payments' ? (
                    <AdminReports />
                  ) : (
                    /* Default fallback for /admin, /admin/dashboard, etc. */
          ...
          <main className="flex-1">
            ...
            {currentPath === '/about' && <AboutUsPage onNavigate={handleNavigate} />}
            ...
            {(currentPath === '/login' ||
              ...
              !['/', '/about', '/services'].includes(currentPath)) && (
              <AuthPage
      ...
      {/* 7. Supabase Database Integration & SQL Command Modal */}
      <SupabaseStatusModal
        isOpen={isSupabaseModalOpen}
        onClose={() => setIsSupabaseModalOpen(false)}
      />

      {/* 8. Profile Picture Camera & Upload Modal */}
      <ProfilePictureModal
